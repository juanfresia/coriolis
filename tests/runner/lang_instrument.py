...
class TestNoOpInstrumenter(unittest.TestCase):
    def test_file(self):
        r = NoOpInstrumenter('/vagrant/resources/pc.chk').can_instrument("")
        self.assertEqual(r, False)

class TestCInstrumenter(unittest.TestCase):
    def test_normal_line(self):
        l = LanguageCInstrumenter('/vagrant/resources/pc.chk').instrument_line('printf("Hello world")')
        self.assertEqual(l, 'printf("Hello world")')

    def test_checkpoint_line(self):
        l = LanguageCInstrumenter('/vagrant/resources/pc.chk').instrument_line('// @checkpoint produce 1 2')
        self.assertEqual(l, 'coriolis_logger_write("%s  %d  %d", "produce", 1, 2);\n')

    def test_checkpoint_not_in_table(self):
        l = LanguageCInstrumenter('/vagrant/resources/pc.chk').instrument_line('// @checkpoint magic 8')
